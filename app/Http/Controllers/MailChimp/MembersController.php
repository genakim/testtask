...
class MembersController extends Controller
{
    ...
    public function remove(string $listId, string $memberId): JsonResponse
    {
        ...
        try {
            ...
            $this->mailChimp->delete(\sprintf('lists/%s/members/%s', $list->getMailChimpId(), $member->getSubscriberHash()));
        } catch (Exception $exception) {
            return $this->errorResponse(['message' => $exception->getMessage()]);
        }
    ...
    public function update(Request $request, string $listId, string  $memberId): JsonResponse
    {
        ...
        try {
            ...
            $this->mailChimp->patch(\sprintf('lists/%s/members/%s', $list->getMailChimpId(), $member->getMailChimpId()),
